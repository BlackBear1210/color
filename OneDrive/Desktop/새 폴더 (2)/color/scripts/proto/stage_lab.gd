...
## ★[2026-07-31] 이 스테이지에서 쓸 수 있는 탄약(페인트) 최대치.
## 오토로드 "탄약"(ammo_manager.gd)에 스테이지 시작 시 리셋해 채워 넣는다 —
## 그래서 스테이지를 넘어가면 항상 이 값으로 잔량이 되돌아온다.
@export var 최대_탄약: int = 10
...
func _ready() -> void:
	...
	# ── 0) 탄약 리셋 ────────────────────────────────────────────────
	# 오토로드라 씬을 넘어가도 값이 남아있다 — 스테이지 시작마다 이 스테이지의
	# 최대치로 다시 채워야 "스테이지 넘어가면 회색·탄약이 회수된다" 규칙이 성립한다.
	탄약.리셋(최대_탄약)
