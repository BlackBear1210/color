...
class 가짜대상 extends Node:
	...
	## 되돌리기() 와 달리 회색이어도 강제로 무색화한다 (실제 지형.gd/통과플랫폼.gd 와 동일한 계약).
	func 강제_초기화() -> void:
		색 = -1
		_맞은 = 0
...
func _검사_리셋() -> void:
	...
	# ★[2026-08-06] 예전엔 회색 대상은 되돌리기()가 false 라 리셋에 안 걸렸는데(버그),
	# 이제 강제_초기화() 로 회색도 리셋 시 무색이 된다.
	확인("회색 대상도 강제 초기화됨", 회색대상.현재색(), -1)
	코어.queue_free()
