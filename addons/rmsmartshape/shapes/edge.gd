...
static func generate_array_mesh_from_quad_sequence(_quads: Array[SS2D_Quad], _wrap_around: bool, color_encoding: int) -> SurfaceTool:
...
## Writes meshes in the given mesh buffer starting at the given index.
...
func get_meshes(color_encoding: SS2D_Edge.COLOR_ENCODING, mesh_buffer: Array[SS2D_Mesh], buffer_idx: int) -> int:
	...
	for consecutive_quads in consecutive_quad_arrays:
		...
		var st := SS2D_Edge.generate_array_mesh_from_quad_sequence(consecutive_quads, wrap_around, color_encoding)
