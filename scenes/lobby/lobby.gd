...
## ★[2026-08-30] "시작" = 집 STAGE 1 (2층 방).
##   월드 라인(world_1 · world_2 · 스마트월드 1~7)은 씬째로 삭제됐다.
##   목적지는 상수로 박지 않고 **챕터표에서 읽는다** — 표가 순서의 유일한 출처다.
...
## (구 `스마트월드_SCENE` 상수는 삭제. 시작 목적지는 `챕터.씬경로(1)` 이다)
...
func _챕터_버튼_추가() -> void:
	...
	#
	# ★[2026-08-30] 표가 집 4 스테이지 전용으로 바뀌었다(월드 라인 전체 삭제).
	#   구 "집 · 2층 방부터"(집_2층방.tscn) 버튼과 "원본 타일맵" 버튼은 없앴다 —
	#   목적지 씬이 사라졌거나, 새 스테이지 라인으로 대체됐다.
	#
	# ⚠ 아직 안 지은 스테이지(2~4)는 **버튼을 만들지 않는다.**
	#   씬 전환은 실패해도 예외를 안 던져서, 없는 씬으로 가면 조용히 검은 화면이 된다.
	#   (2026-08-20 에 같은 이유로 버튼 두 개를 뺀 적이 있다 — 아래 주석 참고)
	for 정보 in 챕터.스테이지표:
...
func _on_start() -> void:
	StageTransition.change_scene(self, 챕터.씬경로(1))
