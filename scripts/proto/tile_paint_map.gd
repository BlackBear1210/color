...
## ★[2026-08-29 신규] 외부 노드(SS2D 지형 등)의 부분 칠이 스스로 흐려져 사라졌을 때
## **그 노드가 직접** 부른다. `페인트_코어.부분_자동회수()` 와 같은 계약이다.
##
## ▣ 왜 필요한가
##   SS2D 지형은 4초 유지 + 1초 감쇠 뒤 스스로 무색으로 돌아간다. 그런데 여기 줄에는
##   "이 노드에 N 발 들어갔다" 가 그대로 남아 있어서, 지형은 비었는데 **탄약은 잠긴 채**가 된다.
##   프로토 계열 스테이지(stage_2-*)에 SS2D 지형을 놓으면 바로 이 상태가 된다.
func 부분_자동회수(대상: Node, 발수: int) -> void:
	var 항목 := _외부_찾기(대상)
	if 항목 == null:
		return
	항목.맞은 = maxi(항목.맞은 - 발수, 0)
	if 항목.맞은 == 0:
		_큐.erase(항목)
	_환급(발수)
	상태변경.emit()
