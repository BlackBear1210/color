...
@export_group("크기")
@export_range(0.6, 2.5, 0.05) var 배율: float = 1.35:
	set(v):
		배율 = v
...
@export_group("초상 담기")
## ▣ "얼굴만 동그랗게 떠 있다"를 고치는 값
##   예전에는 스프라이트의 scale/position 이 씬에 손으로 박혀 있어서, 원이
##   **머리만** 잘라 담았다. 후드와 망토 실루엣이 원 밖으로 나가 안 보였다.
##   → 이제 실제 그림의 **불투명 영역(used_rect)** 을 재서 원에 맞춘다.
##     아트를 갈아 끼워도 프레임 여백이 어떻든 알아서 맞는다.
##
## ▣ 값의 뜻 — **실루엣 세로 중 원 안에 담을 비율**
##   1.00 = 머리부터 발끝까지 전부 담는다 (전신 · 작게 보인다)
##   0.70 = 위에서 70 % 만 담는다 → 다리가 잘리고 **후드 + 망토가 크게** 보인다 (흉상)
##   초상은 "지금 내가 무슨 색인가"를 읽는 자리라 실루엣이 클수록 좋다.
##   대신 너무 자르면 다시 "얼굴만 떠 있는" 예전 문제로 돌아간다.
@export_range(0.4, 1.0, 0.01) var 초상_담을_비율: float = 0.80
## 위/아래 미세 조정 (원 반지름 기준 · 음수면 위로).
@export_range(-0.5, 0.5, 0.01) var 초상_눈높이: float = 0.0
...
## ★[2026-09-05] 초상 원 안에 **실루엣 전체**가 들어가도록 스프라이트를 맞춘다.
##
## 왜 코드로 계산하나 —
##   `player_frames.tres` 의 한 칸은 640 × 640 이지만 캐릭터가 실제로 그려진 곳은
##   그 안의 (139, 101) ~ (434, 558) 이다. 여백이 위아래 좌우로 제각각이라
##   씬에 scale/position 을 손으로 박으면 **아무리 맞춰도 한쪽이 잘린다.**
##   실제로 그렇게 박혀 있었고(얼굴만 보였다), 아트가 바뀌면 또 어긋난다.
##   → 그림에서 불투명 영역을 직접 재서 원에 맞춘다. 딱 한 번(_ready)만 한다.
func _초상_담기() -> void:
	if _초상_그림 == null or _초상_그림.sprite_frames == null:
		return
	var sf := _초상_그림.sprite_frames
	var 애니 := String(_초상_그림.animation)
	if not sf.has_animation(애니):
		애니 = "black_walk"
	if not sf.has_animation(애니) or sf.get_frame_count(애니) <= 0:
		return
	var tex := sf.get_frame_texture(애니, 0)
	if tex == null:
		return
	var 칸 := tex.get_size()
	if 칸.x <= 0.0 or 칸.y <= 0.0:
		return

	# 불투명 영역 = 실루엣이 실제로 차지하는 상자. 못 읽으면 칸 전체로 폴백한다.
	var 내용 := Rect2(Vector2.ZERO, 칸)
	...
	if img != null:
		...
		if r.size.x > 0 and r.size.y > 0:
			내용 = Rect2(r.position, r.size)

	var 원 := (_초상_그림.get_parent() as Node2D)
	var 지름 := 64.0
	var 초상칸 := get_node_or_null("루트/본체/초상") as Control
	if 초상칸:
		지름 = minf(초상칸.size.x, 초상칸.size.y)
	# 원 안에 담을 세로 길이 = 실루엣 세로 × 담을_비율.
	# 그 길이가 원 지름의 92 % 가 되게 키운다(테 두께에 물리지 않을 만큼만 남긴다).
	var 담을세로 := 내용.size.y * clampf(초상_담을_비율, 0.05, 1.0)
	var 배 := 지름 * 0.92 / 담을세로
	# 가로가 원 밖으로 나가면 가로 기준으로 줄인다 — 어깨가 잘리면 실루엣이 안 읽힌다.
	배 = minf(배, 지름 * 0.94 / 내용.size.x)
	_초상_그림.scale = Vector2(배, 배)

	# AnimatedSprite2D 는 칸의 **가운데**를 원점으로 그린다(centered = true).
	# 원 한가운데에 와야 할 것은 칸 중심도 실루엣 중심도 아니라
	# **담을 구간(위에서 담을_비율 만큼)의 중심**이다.
	var 담을중심_y := 내용.position.y + 담을세로 * 0.5
	var 담을중심 := Vector2(내용.position.x + 내용.size.x * 0.5, 담을중심_y)
	var 어긋남 := (담을중심 - 칸 * 0.5) * 배
	var 원중심 := Vector2(지름, 지름) * 0.5 + Vector2(0.0, 지름 * 0.5 * 초상_눈높이)
	# 마스크(Polygon2D)가 원 좌표계이므로 그 안에서의 위치를 그대로 쓴다.
	if 원 != null:
		_초상_그림.position = 원중심 - 어긋남
