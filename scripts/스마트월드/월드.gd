...
func _ready() -> void:
	# ★[2026-08-20] 물리 처리를 **플레이어보다 뒤로** 민다.
	#   경계(색 강제)는 플레이어의 Shift 입력을 **같은 프레임에** 되돌려야 한다.
	#   월드는 Player 의 부모라 기본 순서로는 **입력보다 먼저** 돌아서,
	#   Shift 를 누르면 한 프레임 동안 색이 바뀌었다가 다음 프레임에 되돌아온다 = 깜빡임.
	#   `scenes/경계/color_zone.gd` 가 처음부터 같은 이유로 같은 값을 쓴다.
	#   (식물 B 지대에도 있던 잠재 버그가 이 한 줄로 같이 고쳐진다)
