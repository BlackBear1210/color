...
func _사망_판정() -> bool:
	...
	# 2') 빛기둥 — [2026-08-18 추가] 지붕 구멍·창문에서 새어 드는 빛도 색 경계다.
	#     ★`발판` 모드의 빛은 여기 오지 않는다 — 그건 레이어 1 짜리 단단한 몸이라
	#       아래 3) 지형 겹침 판정이 이미 잡는다(그래서 이 줄을 안 타도 정상이다).
	#       여기서 보는 것은 **통과되는** `경계` 모드뿐이다. 색레이저와 같은 층위다.
	#     그룹으로 물어보는 이유: 빛기둥은 Node2D 라 지형 질의(intersect_shape)에
	#     안 걸린다. 색레이저가 같은 이유로 같은 방식을 쓴다.
