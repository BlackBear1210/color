@tool
extends Area2D
## ============================================================================
## [2026-08-01 신규] 유체 — 물 / 연기 공용
## ----------------------------------------------------------------------------
## ▣ 왜 한 스크립트인가
##   기획서를 보면 물과 연기는 **방향만 반대**고 규칙이 거의 같다.
##     · 흰색/검정/회색이 있다        · 반대색 플레이어와 닿으면 사망
##     · 물리 충돌이 없다(통과)        · 색칠할 수 없다
##     · 서로 섞인다 (검+흰=회 / 검+회=검 / 흰+회=흰)
##   다른 점은 물은 위→아래, 연기는 아래→위로 흐르고, **물만** 반대색 페인트를 지운다.
##   → `종류` 하나로 갈라 쓰는 게 두 파일로 나누는 것보다 규칙이 어긋날 위험이 적다.
##
## ▣ 색 섞임이 왜 저 규칙인가 (기획 그대로)
##   검+흰=회 : 반대색이 만나 중화
##   검+회=검 / 흰+회=흰 : 회색은 "아우르는 색"이라 순색에 흡수된다
##
## ▣ 물리 레이어
##   layer 32 = 유체. 총알(mask 16|32)이 감지해서 반대색이면 막힌다.
##   mask 1|8 = 지형(반대색 페인트를 지우기 위해) + 플레이어(사망 판정)
## ============================================================================
class_name 유체

enum 종류_ { 물, 연기 }

## ColorDefs의 고정값(검정=0, 흰색=1, 회색=2)을 그대로 쓰되,
## 인스펙터에서는 숫자가 아닌 물 색 이름을 선택하게 한다.
enum 물색_ { 흰색 = 1, 회색 = 2, 검정색 = 0 }

## 유체의 물리·혼합 규칙은 그대로 두고, 물만 같은 실루엣의 SpriteFrames 3종으로 보여 준다.
## 색이 바뀔 때 프레임 묶음도 교체해야 회색으로 섞인 물이 이전 색 그림으로 남지 않는다.
const 물_프레임_흰색: SpriteFrames = preload("res://assets/textures/obstacles/liquid/animated_v4/fluid_white_frames_v4.tres")
const 물_프레임_회색: SpriteFrames = preload("res://assets/textures/obstacles/liquid/animated_v4/fluid_gray_frames_v4.tres")
const 물_프레임_검정색: SpriteFrames = preload("res://assets/textures/obstacles/liquid/animated_v4/fluid_black_frames_v4.tres")
## 본체 물결을 바꾸지 않고, 안쪽 물방울만 아래로 움직여 낙하 방향을 읽게 하는 상세 레이어다.
const 물_상세_프레임_흰색: SpriteFrames = preload("res://assets/textures/obstacles/liquid/flow_details_v1/fluid_white_flow_detail_frames_v1.tres")
const 물_상세_프레임_회색: SpriteFrames = preload("res://assets/textures/obstacles/liquid/flow_details_v1/fluid_gray_flow_detail_frames_v1.tres")
const 물_상세_프레임_검정색: SpriteFrames = preload("res://assets/textures/obstacles/liquid/flow_details_v1/fluid_black_flow_detail_frames_v1.tres")

@export var 종류: 종류_ = 종류_.물:
	set(v):
		종류 = v
		_물_그림_갱신()
		queue_redraw()

## ⚠[2026-08-07 버그 수정] 예전에는 `색` 만 바꿨는데, 아래 `_physics_process` 의
##   섞임 계산이 매 프레임 `_원래색` 을 기준으로 색을 다시 정하기 때문에
##   **런타임에 색을 바꾸면 다음 물리 프레임에 원래 색으로 되돌아갔다.**
##   (레버로 물 색을 바꾸거나 도구/테스트가 색을 지정할 때 조용히 무시됐다)
##   → 바깥에서 색을 지정하면 `_원래색` 도 같이 갱신한다. 섞임 계산이 스스로
##     색을 쓸 때만 `_섞는중` 플래그로 그 갱신을 건너뛴다.
@export var 색: 물색_ = 물색_.흰색:
	set(v):
		색 = v
		if not _섞는중:
			_원래색 = v
		_물_그림_갱신()
		queue_redraw()

## 유체가 차지하는 사각 영역(px). 폭 좁고 세로로 길면 "떨어지는 물줄기"가 된다.
@export var 크기: Vector2 = Vector2(64, 260):
	set(v):
		크기 = Vector2(maxf(v.x, 8.0), maxf(v.y, 8.0))
		# 씬을 읽는 중에는 자식 CollisionShape2D가 아직 들어오기 전이다.
		# 그때 만들면 tscn의 같은 이름 노드와 충돌하므로, 로드 완료 뒤에만 갱신한다.
		if is_node_ready():
			_모양_갱신()
		queue_redraw()

## ★[2026-09-02] 판정을 그림보다 조금 **줄이거나 키우는** 유일한 손잡이 (px, 노드 로컬).
##
## ▣ 왜 이것만 두나
##   프레임 8 장의 모양은 그림에서 자동으로 딴 것이라 손으로 고칠 수가 없다
##   (고쳐도 다음 프레임에 덮어써진다). 모양 자체를 바꾸려면 그림을 고치고
##   `tools/생성_유체_판정폴리곤.gd` 를 다시 돌려야 한다.
##   그런데 **맵마다 필요한 난이도 조절**은 그것과 다른 문제다 —
##   "이 물은 스쳐도 안 죽게 조금 얇게" 같은 것. 그래서 외곽선을 통째로
##   안팎으로 밀어내는 값 하나만 인스펙터에 남긴다.
##
##   음수 = 안쪽으로(판정이 얇아진다 · 너그러워진다) · 양수 = 바깥쪽으로(빡빡해진다)
##   0 = 그림 그대로. 인스턴스마다 따로 저장되고 씬을 더럽히지 않는다(그냥 float 하나).
@export_range(-24.0, 24.0, 0.5) var 판정_여유: float = 0.0:
	set(v):
		판정_여유 = clampf(v, -24.0, 24.0)
		if is_node_ready():
			_모양_갱신()
		queue_redraw()

## 흐름 속도(px/s) — 그림이 흐르는 속도. 물리에는 영향 없음.
@export var 흐름속도: float = 130.0

## 이 유체가 켜져 있는가. 제어 배관(레버)이 껐다 켰다 한다.
@export var 켜짐: bool = true:
	set(v):
		켜짐 = v
		_켜짐_반영()

## ★[2026-09-07 신규] 차오르는 물 ─────────────────────────────────────────────
##
## ▣ 기획 (도형님 지시) — "차오르는 물 만들자. 대신 **모든 유체가 차오르게 하는 것이
##   아니라, 인스펙터로 값을 설정할 수 있게** 해서 만들자."
##   → 그래서 `차오름_켜기` 는 **기본 꺼짐**이다. 이 값을 안 건드리면 지금까지
##     놓아둔 유체 수십 개는 한 프레임도 안 바뀐다.
##
## ▣ 왜 위치까지 같이 옮기나 (`_차오름_진행` 의 핵심)
##   유체의 원점은 **물의 윗끝**이고 아래로 `크기.y` 만큼 뻗는다(`_정규화_점_옮기기`).
##   그래서 높이만 키우면 물이 **아래로** 자란다 — 차오르는 게 아니라 흘러내린다.
##   높이를 키운 만큼 원점을 위로 올려야 **바닥이 제자리에 남고 수면만 올라온다.**
##
## ▣ 웅덩이와의 관계
##   같은 손잡이가 `웅덩이.gd` 에도 있다. 이름·기본값·메서드 이름을 일부러 똑같이 맞췄다.
##   레버·버튼은 둘 중 무엇을 물려도 `차오르기_시작()` 한 줄로 동작한다.
@export_group("차오름")
## 켜야 물이 올라온다. 기본은 꺼짐.
@export var 차오름_켜기: bool = false

## 수면이 올라오는 속도(px/s). 60 이면 플레이어 키(약 96px)만큼 오르는 데 1.6 초.
@export_range(1.0, 600.0, 1.0) var 차오름_속도: float = 60.0

## 다 차올랐을 때의 높이(px). `크기.y` 보다 커야 의미가 있다.
@export_range(8.0, 4000.0, 1.0) var 차오름_최종높이: float = 600.0

## 시작 신호를 받고 실제로 오르기 시작할 때까지의 뜸(초).
@export_range(0.0, 20.0, 0.1) var 차오름_시작지연: float = 0.0

## 켜면 스테이지가 시작되자마자 오른다.
## 끄면 레버·버튼이 `차오르기_시작()` 을 부를 때까지 기다린다.
@export var 차오름_자동시작: bool = true

@export_group("낙하 받기")
## 켜면 이 물 위로 떨어져도 낙사하지 않는다(`웅덩이.gd` 와 같은 규칙).
## **기본은 꺼짐** — 유체는 원래 떨어지는 물줄기라, 폭포가 낙하를 받아주면 이상하다.
## 바닥에 눕혀 웅덩이처럼 쓰는 유체에만 켠다. 새로 놓는 웅덩이는 `웅덩이.tscn` 을 쓸 것.
## ⚠ 색이 다르면 낙사 대신 **색으로 죽는다** — 안전망은 색을 맞춘 사람 것이다.
@export var 낙하_받아줌: bool = false

## 이보다 얕으면 못 받아준다(px).
@export_range(0.0, 400.0, 1.0) var 받아주는_최소수심: float = 40.0
@export_group("")

## 판정 폴리곤을 매 프레임 다시 굽지 않기 위한 계단 폭(px).
## 물 판정은 프레임마다 54 점짜리 실루엣을 다시 만드는 비싼 일이라(`_판정_폴리곤들`),
## 60px/s 로 차올라도 초당 30 번만 갱신되게 자른다. 눈으로는 매끈하다.
const 차오름_판정간격: float = 2.0

var _흐름: float = 0.0
var _차오르는중: bool = false
var _차오름_대기: float = 0.0
var _차오름_실제높이: float = 0.0   ## 계단으로 자르기 **전**의 연속 높이
var _차오름_시작높이: float = 0.0   ## 씬에 적어둔 처음 높이 — 리스폰 때 여기로 되돌린다
var _차오름_바닥y: float = 0.0      ## 부모 기준 물의 아랫끝. 차오르는 동안 고정된다
var _원래색: int = ColorDefs.WHITE
var _코어: 페인트코어 = null
var _지운적: Dictionary = {}         ## 같은 지형을 매 프레임 지우지 않도록 기록
## 섞임 계산이 스스로 `색` 을 쓰는 동안만 true. 이때는 `_원래색` 을 건드리면 안 된다
## (건드리면 "섞인 결과"가 기준색이 되어 물이 원래 색으로 못 돌아온다).
var _섞는중: bool = false

## 판정 사각형을 이미 내 것으로 복제했다는 표식. `_직사각_판정()` 참고.
const 내_모양_표식 := &"__유체_전용_판정모양__"

## ★[2026-09-02] 물 판정을 **그림 실루엣 폴리곤**으로 바꿨다 (성진님 지시).
##
## ▣ 예전에는 직사각형 3 개였다 — 물줄기 / 중간 물보라 / 바닥 물보라.
##   그림은 좌우로 굽이치는 물기둥인데 판정은 곧은 기둥이라,
##   물이 왼쪽으로 휜 프레임에서는 **오른쪽 빈 공간에서 죽고** 휜 쪽 물은 스쳐도 안 죽었다.
##   "보이는 것과 닿는 것이 다르다" 는 색이 곧 규칙인 이 게임에서 가장 나쁜 거짓말이다.
##
## ▣ 지금은 `유체판정모양.프레임(n)` 이 프레임마다 다른 정규화 폴리곤을 준다.
##   그 표는 `tools/생성_유체_판정폴리곤.gd` 가 그림의 알파에서 직접 딴 것이고,
##   알파문턱 0.35 라 흐릿한 가장자리는 자연히 판정에서 빠진다(스쳐 죽는 것 방지).
##   튀는 물방울(3~10px)은 버렸다 — 안 보이는 점 하나에 죽으면 안 되니까.
##
## ▣ 판정 노드는 **씬에 저장하지 않는다.**
##   `polygon` 은 리소스가 아니라 노드 속성이라, 에디터에서 이 스크립트가 값을 넣으면
##   유체마다 54 점짜리 덮어쓰기가 stage 씬에 저장되어 파일이 수천 줄 불어난다.
##   → 씬에는 두지 않고 여기서 만든다(owner 없음 = 어느 씬에도 안 실린다).
##     덤으로 2026-09-02 의 "인스턴스가 Shape 리소스를 나눠 쓴다" 버그가 구조적으로 못 생긴다.
const 물_판정_이름_앞머리 := "물_판정_"
## 예전 판(직사각형 3 개)이 저장된 씬을 만나면 꺼야 한다. 안 끄면 이중으로 판정된다.
const 옛_판정_이름들: Array[StringName] = [&"물줄기_판정", &"중간_물보라_판정", &"바닥_물보라_판정"]
var _마지막_판정_프레임: int = -1


func _ready() -> void:
	collision_layer = 32
	collision_mask = 1 | 8
	monitoring = true
	monitorable = true
	_모양_갱신()
	_원래색 = 색
	_물_그림_갱신()
	if Engine.is_editor_hint():
		queue_redraw()
		return
	add_to_group("유체")
	_코어 = get_tree().get_first_node_in_group("페인트코어") as 페인트코어
	# ★[2026-09-07] 차오름 기준값은 **씬에 적힌 처음 값**으로 잡는다.
	#   리스폰 때 여기로 되돌리지 않으면 물이 가득 찬 방으로 되살아나 무한히 죽는다.
	_차오름_시작높이 = 크기.y
	_차오름_실제높이 = 크기.y
	_차오름_바닥y = position.y + 크기.y
	if 차오름_켜기:
		add_to_group("차오르는물")     # 월드.gd 가 리스폰 때 이 그룹만 초기화한다
		if 차오름_자동시작:
			차오르기_시작()
	if 낙하_받아줌:
		add_to_group("낙하받이")       # 월드.gd 가 낙사 직전에 이 그룹만 훑는다
	set_physics_process(true)
	set_process(true)
	_켜짐_반영()


func _모양_갱신() -> void:
	if 종류 == 종류_.물:
		_물_판정_갱신()
	else:
		_물_판정_숨기기()
		var c := _직사각_판정("모양")
		var s := c.shape as RectangleShape2D
		s.size = 크기
		# 연기는 기존 직사각형 판정을 유지한다. 물 실루엣 폴리곤은 **떨어지는 물**의 모양이라
		# 위로 퍼지는 기체에 씌우면 보이는 범위와 판정이 오히려 어긋난다.
		# (연기 그림이 생기면 그때 같은 방식으로 연기 실루엣을 따로 구우면 된다)
		c.disabled = false
		c.position = Vector2(0, -크기.y * 0.5)
	# 유체 인스턴스마다 물줄기 길이가 다르다. 그림도 같은 판정 크기로 맞춰야
	# 56x300 기본값 그림이 긴 물줄기 중앙에만 짧게 남지 않는다.
	var 그림 := 아트슬롯.슬롯(self)
	if 그림 != null:
		그림.기준_크기 = 크기
	_물_애니_크기_맞추기()


## 지금 그려지고 있는 프레임의 실루엣을 그대로 충돌 폴리곤으로 넣는다.
## 원점은 배관 출구와 맞추기 위한 물의 윗끝이다(y 0 → 크기.y 가 물의 아랫끝).
func _물_판정_갱신() -> void:
	var 이전_사각 := get_node_or_null("모양") as CollisionShape2D
	if 이전_사각 != null and not 이전_사각.disabled:
		# 연기용(또는 옛 판) 전체 사각형이 남아 있어도 중복 판정되지 않게 끈다.
		# (이미 꺼져 있으면 건드리지 않는다 — 아래 `묶음` 루프의 주석과 같은 이유)
		이전_사각.disabled = true
	_옛_사각_판정_끄기()

	# ⚠[2026-09-07] `polygon` 과 `disabled` 는 **값이 바뀔 때만** 쓴다.
	#   둘 다 물리 서버에서 `shape_owner_clear_shapes()` → 다시 달기를 일으키고,
	#   도형이 지워지는 순간 Area2D 겹침 쌍이 끊어진다. 안 바뀐 값을 다시 써 넣으면
	#   **아무 이유 없이 겹침이 한 프레임 끊긴다** — 호퍼가 그걸 "물이 끊겼다"로 읽어
	#   출구 물이 점멸했다(`호퍼.gd` 의 `끊김_유예` 주석에 전말이 있다).
	#   모양이 진짜로 바뀌는 프레임은 어쩔 수 없지만, 안 바뀌는 것까지 흔들 이유는 없다.
	var 묶음 := _판정_폴리곤들(_현재_프레임())
	for i in 유체판정모양.최대_섬:
		if i < 묶음.size():
			var 판정 := _폴리곤_판정(i)
			if 판정.polygon != 묶음[i]:
				판정.polygon = 묶음[i]
			if 판정.disabled:
				판정.disabled = false
			continue
		# 프레임마다 섬 개수가 다르다. 남는 슬롯은 **있을 때만** 비우고 끈다.
		# (없으면 만들지 않는다 — 쓰지도 않을 노드를 유체마다 3 개씩 달 이유가 없다)
		var 남는 := get_node_or_null(NodePath(물_판정_이름_앞머리 + str(i))) as CollisionPolygon2D
		if 남는 != null and not 남는.disabled:
			남는.polygon = PackedVector2Array()
			남는.disabled = true


func _물_판정_숨기기() -> void:
	_옛_사각_판정_끄기()
	for i in 유체판정모양.최대_섬:
		var 판정 := get_node_or_null(NodePath(물_판정_이름_앞머리 + str(i))) as CollisionPolygon2D
		if 판정 != null:
			판정.disabled = true


func _옛_사각_판정_끄기() -> void:
	for 이름 in 옛_판정_이름들:
		var 판정 := get_node_or_null(NodePath(이름)) as CollisionShape2D
		if 판정 != null:
			판정.disabled = true


## 한 프레임의 판정 폴리곤(노드 로컬 px). `판정_여유` 까지 반영한 **최종** 모양이다.
## 판정과 에디터 미리보기 선이 반드시 같은 것을 쓰도록 여기 한 곳에서만 만든다.
func _판정_폴리곤들(프레임: int) -> Array:
	var 결과: Array = []
	for 정규: PackedVector2Array in 유체판정모양.프레임(프레임):
		var 로컬 := _정규화_점_옮기기(정규)
		if is_zero_approx(판정_여유):
			결과.append(로컬)
			continue
		# 안쪽으로 너무 밀면 폴리곤이 사라지거나 여러 조각으로 갈라진다.
		# 사라지면 판정이 통째로 없어져 "물에 들어가도 안 죽는" 상태가 되므로 원본으로 되돌린다.
		var 민것 := Geometry2D.offset_polygon(로컬, 판정_여유, Geometry2D.JOIN_ROUND)
		if 민것.is_empty():
			결과.append(로컬)
			continue
		for 조각: PackedVector2Array in 민것:
			if 조각.size() >= 3:
				결과.append(조각)
	# 슬롯 개수(=최대_섬)를 넘으면 물리에 못 넣는다. 큰 조각부터 남긴다.
	if 결과.size() > 유체판정모양.최대_섬:
		결과.sort_custom(func(a, b): return _넓이(a) > _넓이(b))
		결과 = 결과.slice(0, 유체판정모양.최대_섬)
	return 결과


func _넓이(p: PackedVector2Array) -> float:
	var s := 0.0
	for i in p.size():
		var a := p[i]
		var b := p[(i + 1) % p.size()]
		s += a.x * b.y - b.x * a.y
	return absf(s) * 0.5


## 정규화 좌표(u,v) → 노드 로컬 px.
## 그림은 `_물_애니_크기_맞추기()` 에서 position (0, 크기.y/2) · scale (크기.x/256, 크기.y/512)
## 로 놓이고 AnimatedSprite2D 는 중앙 정렬이다. 그래서 텍스처 u=0.5 가 로컬 x=0,
## v=0 이 로컬 y=0 이 된다 — 아래 식이 그림과 판정을 정확히 같은 자리에 놓는 이유다.
func _정규화_점_옮기기(정규화: PackedVector2Array) -> PackedVector2Array:
	var 결과 := PackedVector2Array()
	결과.resize(정규화.size())
	for i in 정규화.size():
		결과[i] = Vector2((정규화[i].x - 0.5) * 크기.x, 정규화[i].y * 크기.y)
	return 결과


## ⚠ 씬에 두지 않고 여기서 만든다. 이유는 위 `물_판정_이름_앞머리` 주석 참고.
##   owner 를 안 주므로 이 노드는 어느 .tscn 에도 저장되지 않는다.
func _폴리곤_판정(번호: int) -> CollisionPolygon2D:
	var 이름 := 물_판정_이름_앞머리 + str(번호)
	var 판정 := get_node_or_null(NodePath(이름)) as CollisionPolygon2D
	if 판정 == null:
		판정 = CollisionPolygon2D.new()
		판정.name = 이름
		# SOLIDS 라야 "물 안에 들어왔다" 는 겹침 판정이 된다.
		# SEGMENTS 는 선분뿐이라 안쪽이 없어 Area2D 가 사람을 못 잡는다.
		판정.build_mode = CollisionPolygon2D.BUILD_SOLIDS
		add_child(판정)
	return 판정


## ⚠[2026-09-02] 이 판정 사각형은 **인스턴스마다 자기 것**이어야 한다.
##
## ▣ 무슨 일이 있었나 (stage_2-1 물 판정이 그림과 어긋남)
##   씬 파일에 박힌 서브리소스는 `resource_local_to_scene` 이 꺼져 있으면
##   **그 씬의 모든 인스턴스가 한 개를 공유한다** — Godot 의 기본 동작이다.
##   아래에서 `모양.size` 를 제자리에서 고치기 때문에, 공유된 채로 두면
##   유체 14 개가 사각형 하나에 번갈아 써서 **맨 마지막 것이 이긴다.**
##   실제로 stage_2-1 의 물 14 개가 크기가 제각각(56×300 ~ 300×800)인데도
##   판정은 전부 `18.592 × 675` 한 값이었다(= 마지막 노드 유체6 의 값).
##   `position` 만 멀쩡했던 이유는 그건 리소스가 아니라 **노드 속성**이라서다.
##   → 그림은 각자 크기대로 그려지는데 판정만 딴 데 있으니, 폭 300 짜리 물이
##     18.6px 로만 잡히는 등 "물에 닿았는데 안 죽는" 상태가 된다.
##
## ▣ 지금은 어떻게 되어 있나
##   ★[2026-09-02 저녁] **물**은 이 함수를 안 쓴다 — 실루엣 폴리곤으로 바뀌었다.
##     `polygon` 은 리소스가 아니라 노드 속성이라 애초에 공유될 것이 없다.
##     그래서 `유체.tscn` 에서 사각형 3 개와 그 서브리소스를 아예 지웠다.
##   남은 사용처는 **연기**의 `모양` 하나뿐이고, 그건 씬에 없으니 여기서 새로 만든다
##   (= 인스턴스마다 자기 것). 아래 복제 코드는 옛 씬에 `모양` 이 저장돼 있을 때의 안전망이다.
##   ⚠ 런타임에서만 복제한다 — 에디터에서 `판정.shape` 에 새 리소스를 넣으면
##     그게 인스턴스 덮어쓰기로 **씬 파일에 저장**되어 파일이 지저분해진다.
func _직사각_판정(이름: StringName) -> CollisionShape2D:
	var 판정 := get_node_or_null(NodePath(이름)) as CollisionShape2D
	if 판정 == null:
		판정 = CollisionShape2D.new()
		판정.name = 이름
		add_child(판정)
	var 모양 := 판정.shape as RectangleShape2D
	if 모양 == null:
		모양 = RectangleShape2D.new()
		판정.shape = 모양
	elif not (Engine.is_editor_hint() or 모양.resource_local_to_scene
			or 판정.has_meta(내_모양_표식)):
		# 남과 나눠 쓰는 사각형이다 → 내 것으로 복제한다. 한 번만 하면 된다.
		모양 = 모양.duplicate() as RectangleShape2D
		판정.shape = 모양
		판정.set_meta(내_모양_표식, true)
	return 판정


func _켜짐_반영() -> void:
	monitoring = 켜짐
	visible = 켜짐


## 물은 반투명 SpriteFrames 3종을 색 규칙과 같은 기준으로 골라 계속 재생한다.
## 연기는 기존 코드 그림을 유지한다. 물 프레임을 억지로 뒤집어 쓰면 기체처럼 안 보이기 때문이다.
func _물_그림_갱신() -> void:
	var 그림 := 아트슬롯.슬롯(self)
	var 애니 := _물_애니()
	var 상세 := _물_상세_애니()
	if 종류 != 종류_.물:
		# 연기용 커스텀 그림은 보존하고, 물 애니메이션만 숨긴다.
		if 그림 != null:
			그림.visible = true
		if 애니 != null:
			애니.stop()
			애니.visible = false
		if 상세 != null:
			상세.stop()
			상세.visible = false
		return
	# 물은 정지 Sprite2D 대신 AnimatedSprite2D가 그린다. 그래야 _draw가 아래에 겹쳐 그리지 않는다.
	if 그림 != null:
		그림.visible = false
	if 애니 == null and 상세 == null:
		return
	match 색:
		ColorDefs.BLACK:
			if 애니 != null: 애니.sprite_frames = 물_프레임_검정색
			if 상세 != null: 상세.sprite_frames = 물_상세_프레임_검정색
		ColorDefs.GRAY:
			if 애니 != null: 애니.sprite_frames = 물_프레임_회색
			if 상세 != null: 상세.sprite_frames = 물_상세_프레임_회색
		_:
			if 애니 != null: 애니.sprite_frames = 물_프레임_흰색
			if 상세 != null: 상세.sprite_frames = 물_상세_프레임_흰색
	if 애니 != null:
		애니.animation = &"흐름"
		애니.visible = true
	if 상세 != null:
		상세.animation = &"흐름"
		상세.visible = true
	_물_애니_크기_맞추기()
	# 에디터에서는 첫 프레임만 보이고, 게임에서는 8fps 루프가 바로 이어서 흐른다.
	if Engine.is_editor_hint():
		if 애니 != null: 애니.frame = 0
		if 상세 != null: 상세.frame = 0
	else:
		if 애니 != null: 애니.play(&"흐름")
		if 상세 != null: 상세.play(&"흐름")


## 원점은 물의 출구(윗끝)다. 프레임 중앙을 판정 사각형 중앙으로 옮긴 뒤 같은 크기로 맞춘다.
func _물_애니_크기_맞추기() -> void:
	var 애니 := _물_애니()
	var 상세 := _물_상세_애니()
	if 종류 != 종류_.물:
		return
	var 배율 := Vector2(크기.x / 256.0, 크기.y / 512.0)
	if 애니 != null:
		애니.position = Vector2(0, 크기.y * 0.5)
		애니.scale = 배율
	if 상세 != null:
		상세.position = Vector2(0, 크기.y * 0.5)
		상세.scale = 배율


func _물_애니() -> AnimatedSprite2D:
	return get_node_or_null("물_애니메이션") as AnimatedSprite2D


func _물_상세_애니() -> AnimatedSprite2D:
	return get_node_or_null("물결_상세_애니메이션") as AnimatedSprite2D


func _물_애니_재생중() -> bool:
	var 애니 := _물_애니()
	return 애니 != null and 애니.visible and 애니.sprite_frames != null


func _현재_프레임() -> int:
	var 애니 := _물_애니()
	if 애니 == null:
		return 0
	return clampi(애니.frame, 0, 유체판정모양.프레임_수 - 1)


## AnimatedSprite2D의 프레임이 바뀐 때만 충돌 폭을 갱신한다.
## 매 렌더 프레임에 Shape2D를 새로 쓰지 않아도 8fps 물결과 판정이 정확히 같이 움직인다.
func _물_애니_판정_따르기() -> void:
	if 종류 != 종류_.물:
		return
	var 애니 := _물_애니()
	if 애니 == null or not 애니.visible or 애니.frame == _마지막_판정_프레임:
		return
	_마지막_판정_프레임 = 애니.frame
	_물_판정_갱신()


# ── 규칙 ───────────────────────────────────────────────────────────────────
## 반대색 투사체를 막는다 (기획: "물과 상반되는 색의 투사체를 막는다").
## 회색 유체는 아무것도 안 막는다 — 회색은 두 색을 아우르는 색이라서.
func 총알_막나(총알색: int) -> bool:
	if not 켜짐 or 색 == ColorDefs.GRAY:
		return false
	return 총알색 != 색


## 플레이어가 이 유체에 닿으면 죽는가.
func 반대색인가(플레이어색: int) -> bool:
	# 꺼진 유체는 그 자리에 아무것도 없다 → 색 규칙 이전의 문제다.
	if not 켜짐:
		return false
	# ★[2026-08-30] 규칙은 `색규칙.gd` 한 곳에서만 판단한다.
	return 색규칙.위험한가(색, 플레이어색)


## 유체는 색칠할 수 없다 → 총알이 맞아도 페인트는 회수된다.
func 명중(_색: int, _월드좌표: Vector2) -> String:
	return "blocked"


func 되돌리기() -> bool:
	return false


func 현재색() -> int:
	return 색


## 물은 원점에서 아래로 시작한다. 호퍼·저장고 포트가 이 점을 기준으로 실제 연결을 찾는다.
func 시작점_월드좌표() -> Vector2:
	return global_position


# ── 차오름 ──────────────────────────────────────────────────────────────────
# ⚠ 아래 네 함수는 `웅덩이.gd` 의 같은 이름 함수와 **똑같이 동작해야 한다.**
#   레버·버튼은 둘 중 무엇이 물려 있는지 모르고 `차오르기_시작()` 만 부른다.

## 레버·압력버튼·통로 트리거가 이걸 부르면 물이 오르기 시작한다.
func 차오르기_시작() -> void:
	if not 차오름_켜기:
		return
	# ⚠ 아직 한 번도 안 차올랐으면 **지금 자리·지금 높이**로 기준을 다시 잡는다.
	#   `_ready` 때 잡은 값만 믿으면, 인스턴스를 만든 **뒤에** 위치·크기를 넣는 경우
	#   (도구·코드로 배치할 때가 그렇다) 엉뚱한 바닥으로 순간이동한다.
	#   한 번 차오른 뒤에는 안 건드린다 — 그래야 `차오르기_초기화()` 가 제자리로 돌아온다.
	if not _차오르는중 and is_equal_approx(_차오름_실제높이, _차오름_시작높이):
		_차오름_시작높이 = 크기.y
		_차오름_실제높이 = 크기.y
		_차오름_바닥y = position.y + 크기.y
	_차오르는중 = true
	_차오름_대기 = 차오름_시작지연


## 지금 높이에서 멈춘다. 다시 `차오르기_시작()` 하면 그 자리에서 이어서 오른다.
func 차오르기_멈춤() -> void:
	_차오르는중 = false


## 처음 높이·처음 자리로 되돌린다. **리스폰 때 반드시 불러야 한다.**
func 차오르기_초기화() -> void:
	_차오르는중 = false
	_차오름_대기 = 0.0
	_차오름_실제높이 = _차오름_시작높이
	position.y = _차오름_바닥y - _차오름_시작높이
	크기 = Vector2(크기.x, _차오름_시작높이)
	if 차오름_켜기 and 차오름_자동시작:
		차오르기_시작()


## 바닥을 고정한 채 수면만 올린다. 위 `차오름_켜기` 머리말의 "왜 위치까지 옮기나" 참고.
func _차오름_진행(delta: float) -> void:
	if not _차오르는중:
		return
	if _차오름_대기 > 0.0:
		_차오름_대기 = maxf(_차오름_대기 - delta, 0.0)
		return
	var 목표 := maxf(차오름_최종높이, _차오름_시작높이)
	if _차오름_실제높이 >= 목표 - 0.01:
		_차오르는중 = false
		return
	_차오름_실제높이 = minf(_차오름_실제높이 + 차오름_속도 * delta, 목표)
	var 계단 := clampf(snappedf(_차오름_실제높이, 차오름_판정간격), _차오름_시작높이, 목표)
	if absf(계단 - 크기.y) < 0.01:
		return
	# 위치를 먼저 올리고 높이를 키운다. 순서를 바꾸면 한 프레임 동안 물이 아래로 늘어난다.
	position.y = _차오름_바닥y - 계단
	크기 = Vector2(크기.x, 계단)


## 지금 수면의 월드 y (작을수록 높다). 물의 원점이 곧 수면이다.
func 수면_월드y() -> float:
	return global_position.y


func 수심() -> float:
	return 크기.y


## 월드.gd 가 낙사 직전에 물어본다. 색은 여기서 안 본다 —
## 색이 다르면 어차피 `반대색인가()` 가 죽인다. 두 판정을 섞으면 사인이 헷갈린다.
func 낙하를_받아주나() -> bool:
	return 켜짐 and 낙하_받아줌 and 크기.y >= 받아주는_최소수심


## 다른 유체와 섞였을 때의 결과색. 기획의 표를 그대로 옮겼다.
static func 섞기(a: int, b: int) -> int:
	if a == b:
		return a
	# 검 + 흰 = 회
	if (a == ColorDefs.BLACK and b == ColorDefs.WHITE) or (a == ColorDefs.WHITE and b == ColorDefs.BLACK):
		return ColorDefs.GRAY
	# 검 + 회 = 검 / 흰 + 회 = 흰  (순색이 회색을 흡수)
	return a if b == ColorDefs.GRAY else b


func _physics_process(delta: float) -> void:
	if Engine.is_editor_hint() or not 켜짐:
		return

	# ── 0) 차오르기 (켜 둔 유체만) ──
	_차오름_진행(delta)

	# ── 1) 다른 유체와 섞이기 ──
	var 섞인색 := _원래색
	for 영역 in get_overlapping_areas():
		var f := 영역 as 유체
		if f and f.켜짐 and f.종류 == 종류:
			섞인색 = 유체.섞기(섞인색, f._원래색)
	if 섞인색 != 색:
		_섞는중 = true
		색 = 섞인색
		_섞는중 = false

	# ── 2) 물만: 닿은 지형의 반대색 페인트를 지운다 (지워진 색은 회수된다) ──
	if 종류 != 종류_.물 or 색 == ColorDefs.GRAY:
		return
	for 바디 in get_overlapping_bodies():
		var 대상 := _칠할대상_찾기(바디)
		if 대상 == null:
			continue
		if 대상.has_method("물에_안지워짐") and 대상.물에_안지워짐():
			continue                              # 통과 플랫폼은 물에 안 지워진다
		if not 대상.has_method("현재색") or not 대상.has_method("되돌리기"):
			continue
		var 대상색: int = 대상.현재색()
		# 반대색만 지운다. 같은 색·무색·회색은 그대로 둔다.
		if 대상색 < 0 or 대상색 == ColorDefs.GRAY or 대상색 == 색:
			continue
		if _지운적.has(대상):
			continue
		if 대상.되돌리기():
			_지운적[대상] = true
			if _코어:
				# 지워진 색은 회수된다 = 탄약이 돌아온다
				_코어.부분_자동회수(대상, 1)


func _칠할대상_찾기(바디: Object) -> Node:
	var n := 바디 as Node
	while n != null:
		if n.has_method("되돌리기"):
			return n
		n = n.get_parent()
	return null


# ── 그림 ───────────────────────────────────────────────────────────────────
func _process(delta: float) -> void:
	if not 켜짐:
		return
	_흐름 += delta * 흐름속도
	_물_애니_판정_따르기()
	queue_redraw()


func _draw() -> void:
	# ★[2026-09-02] 에디터에서만 판정 외곽선을 겹쳐 그린다.
	#   판정 노드를 씬에 안 두는 대신(위 `물_판정_이름_앞머리` 주석) 이걸로 눈으로 확인한다.
	#   디자이너가 `크기` 를 바꿔도 그림과 선이 같이 움직이는지 바로 보인다.
	if Engine.is_editor_hint() and 켜짐 and 종류 == 종류_.물:
		for 섬: PackedVector2Array in _판정_폴리곤들(_현재_프레임()):
			var 선 := PackedVector2Array(섬)
			선.append(선[0])                      # 닫아 준다
			draw_polyline(선, Color(1.0, 0.25, 0.15, 0.85), 1.5)

	# ── [2026-08-07 도형] 디자이너 그림 슬롯 ────────────────────────────
	# 자식 `그림`(아트슬롯.gd) 또는 물 애니메이션이 보이면 코드 그리기는 쉰다.
	# 둘 다 비었을 때만 기존 도형 그림을 써서, 연기와 커스텀 그림의 회귀를 막는다.
	if 아트슬롯.그림_있나(self) or _물_애니_재생중():
		return

	if not 켜짐:
		return
	var 기본 := Color(0.55, 0.58, 0.62)
	match 색:
		ColorDefs.BLACK: 기본 = Color(0.10, 0.11, 0.14)
		ColorDefs.WHITE: 기본 = Color(0.90, 0.93, 0.95)
		ColorDefs.GRAY:  기본 = Color(0.50, 0.51, 0.52)

	var 위 := 0.0 if 종류 == 종류_.물 else -크기.y
	var 아래 := 위 + 크기.y
	var 반 := 크기.x * 0.5
	var 방향 := 1.0 if 종류 == 종류_.물 else -1.0

	# ── 본체 ──
	# 예전에는 사각형 + 가로 줄무늬였는데 사다리처럼 보였다.
	# 좌우 가장자리를 사인으로 흔든 폴리곤으로 바꾸니 "흐르는 기둥"으로 읽힌다.
	var 칸 := 16
	var 왼: PackedVector2Array = PackedVector2Array()
	var 오른: PackedVector2Array = PackedVector2Array()
	for i in 칸 + 1:
		var t := float(i) / float(칸)
		var y := lerpf(위, 아래, t)
		# 위치마다 위상이 다른 사인 두 개 → 규칙적이지 않은 물결
		var w := 반 * (1.0 + sin(y * 0.05 + _흐름 * 0.04 * 방향) * 0.10
			+ sin(y * 0.11 - _흐름 * 0.07 * 방향) * 0.06)
		왼.append(Vector2(-w, y))
		오른.append(Vector2(w, y))
	var 본체 := PackedVector2Array(왼)
	for i in range(오른.size() - 1, -1, -1):
		본체.append(오른[i])
	# 연기는 더 옅게 — 기체라서
	var 진하기 := 0.34 if 종류 == 종류_.물 else 0.22
	draw_colored_polygon(본체, Color(기본.r, 기본.g, 기본.b, 진하기))

	# ── 흐름 결 ──
	# 세로로 길게 늘어진 가는 선을 스크롤시킨다. 가로 줄무늬보다 훨씬 "흐른다"는 느낌.
	var 결수 := 5
	for i in 결수:
		var fx := lerpf(-반 * 0.62, 반 * 0.62, float(i) / float(결수 - 1))
		var 길이 := 크기.y * 0.30
		var 시작 := fmod(_흐름 * 방향 * (0.8 + float(i) * 0.13) + float(i) * 97.0, 크기.y + 길이)
		if 시작 < 0.0:
			시작 += 크기.y + 길이
		var y0 := clampf(위 + 시작 - 길이, 위, 아래)
		var y1 := clampf(위 + 시작, 위, 아래)
		if y1 - y0 > 2.0:
			draw_line(Vector2(fx, y0), Vector2(fx, y1),
				Color(기본.r, 기본.g, 기본.b, 0.30), 3.0)

	# ── 가장자리 하이라이트 — 유리 같은 굴절 느낌 ──
	draw_polyline(왼, Color(1, 1, 1, 0.20), 2.0)
	draw_polyline(오른, Color(1, 1, 1, 0.12), 2.0)
