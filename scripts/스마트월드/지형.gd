...
## ★[2026-09-07 신규] 이 지형에 들어온 발 중 **장치(페인트 분사기)가 쏜 발** 수. [검정, 흰색]
##
## ▣ 왜 세나 — 안 세면 없는 탄약이 생긴다
##   부분 칠은 4초 뒤 흐려지면서 `_코어.부분_자동회수()` 로 **발수를 돌려준다.**
##   그런데 장치가 쏜 발은 플레이어가 낸 적이 없다. 그대로 두면 분사기 앞에
##   가만히 서 있는 것만으로 탄약이 계속 불어난다.
##   → 만료될 때 장치 몫을 먼저 빼고, 남은 것(=플레이어가 낸 발)만 환급한다.
var _장치발수: Array[int] = [0, 0]
...
var _긴변: float = 200.0                     ## 로컬 AABB 의 긴 변 — 얼룩 계산에 쓴다
## ★월드 크기의 긴 변. **게임 규칙(필요횟수·전체 색칠 한도)은 이걸 본다.**
var _세계긴변: float = 200.0
...
func _로컬반경_계산() -> void:
	...
	# ★[2026-09-07] 규칙은 **월드 크기**로 재야 한다.
	#   Template 을 축척해서 꽂은 지형은 로컬 긴변이 Template 원본(시멘트 1,731px)이라,
	#   실제로는 180px 짜리 작은 발판인데도 한도(576)를 넘어 `전체_색칠_가능()` 이 false 가
	#   되고 **아무리 쏴도 안 굳었다** — 하수도 2-2 관문 4 의 유령 다리 3장이 그랬다.
	#   점 배열을 월드 좌표로 직접 찍은 지형은 축척이 1 이라 값이 그대로다(기존 동작 유지).
...
## ★[2026-09-07 신규] 장치(페인트 분사기)가 쏜 페인트의 명중.
## 칠하는 규칙은 플레이어 총알과 **완전히 같다** — 다른 건 "환급 대상이 아니다" 하나뿐이다.
func 장치_명중(색: int, 월드좌표: Vector2) -> String:
	var 결과 := 명중(색, 월드좌표)
	match 결과:
		"progress":
			if 색 >= ColorDefs.BLACK and 색 <= ColorDefs.WHITE:
				_장치발수[색] += 1
		"painted":
			# 전체 색칠로 승격되면 `_진행` 이 비워져 만료(=환급) 자체가 안 일어난다.
			# 남은 장치 몫을 들고 있으면 다음 부분 칠의 환급을 잘못 깎으므로 같이 턴다.
			_장치발수 = [0, 0]
	return 결과
...
func _process(delta: float) -> void:
	...
	if _진행.전체횟수() > 0:
		...
		for 만료색 in 변화["만료"]:
			var c := int(만료색)
			_색_시드_지우기(c)
			# 장치(분사기)가 넣은 발은 플레이어 탄약이 아니라 환급에서 먼저 뺀다.
			var 만료수 := int(변화["만료"][만료색])
			var 장치몫 := mini(_장치발수[c], 만료수) if c >= 0 and c < 2 else 0
			if 장치몫 > 0:
				_장치발수[c] -= 장치몫
			var 돌려줄 := 만료수 - 장치몫
			if 돌려줄 > 0 and _코어 and _코어.has_method("부분_자동회수"):
				_코어.부분_자동회수(self, 돌려줄)
		_맞은횟수 = _진행.전체횟수()
		var 완성색: int = 변화["완성색"]
