...
## ★켜면 이 노드를 **플레이어 앞**(기본 z_index +40)에 그린다.
##   원화의 "앞쪽 바닥 띠"만 `그림_원본영역` 으로 떠서 한 겹 더 깔면, 플레이어가 그 띠
##   뒤로 들어가며 방에 **깊이**가 생긴다(2.5D). 끄면 예전대로 맨 뒤(z −60).
##
## ⚠★[2026-09-03] 이 체크박스는 이제 **아래 `깊이` 의 편의 스위치**일 뿐이다.
##   켜면 깊이 = +40, 끄면 깊이 = −60 을 넣는다. 세밀하게 정하고 싶으면 `깊이` 를 직접 쓴다.
@export var 앞에_그리기: bool = false:
	set(v):
		앞에_그리기 = v
		깊이 = 40 if 앞에_그리기 else -60

## ★★[2026-09-03 도형님 "앞바닥 지형을 깔면 배경이 자꾸 앞으로 나온다 → 뒤로 빼줘"]
## ----------------------------------------------------------------------------
## ▣ 무엇이 문제였나
##   예전에는 `앞에_그리기` 체크 하나로 깊이가 **−60(맨 뒤) 아니면 +40(플레이어 앞)**
##   두 값뿐이었다. 그래서 원화의 앞바닥 띠를 한 겹 깔면 그 띠가 지형·플레이어를
##   **통째로 덮는다.** STAGE 2 의 `앞바닥_1층` 이 정확히 그 상태였다 —
##   앞쪽에 바닥 지형을 놓을수록 배경이 그 위로 올라와 보인다.
##   "벽 원화보다는 앞, 지형보다는 뒤" 라는 **가운데 자리**를 잡을 방법이 아예 없었다.
##
## ▣ 그래서 z_index 를 숫자로 직접 적게 했다
##   지형(SS2D)은 z 0 이다. 그러므로 **음수를 넣으면 무조건 지형 뒤**로 간다.
##
##   | 값        | 자리                                        |
##   |-----------|---------------------------------------------|
##   | −60       | 맨 뒤 — 방 벽 원화. **기본값**              |
##   | −59 ~ −20 | 벽 원화 앞 · **지형 뒤**. 앞바닥 띠는 여기  |
##   | −8 ~ −1   | 지형 바로 뒤(문간 어둠 등 얇은 덧칠)        |
##   | +40       | 플레이어 앞(2.5D 전경) ⚠ 지형을 가린다      |
##
## ⚠ 선언 순서가 `앞에_그리기` **다음**이어야 한다. 씬은 선언 순서대로 값을 되살리므로,
##   뒤에 오는 `깊이` 가 앞 체크박스의 −60/+40 을 덮어써 항상 이긴다. 순서를 바꾸면
##   저장해 둔 깊이가 체크박스 값으로 되돌아간다.
@export_range(-100, 100) var 깊이: int = -60:
	set(v):
		깊이 = v
		z_index = 깊이
...
func _ready() -> void:
	...
	# ★[2026-09-03] 이제 깊이는 `깊이`(z_index) 한 값이 정한다.
	#   `앞에_그리기` 는 그 값을 −60/+40 으로 채워 넣는 편의 스위치일 뿐이라
	#   여기서 다시 계산하면 씬에 저장된 중간값(예: 앞바닥 −20)을 덮어써 버린다.
	z_index = 깊이
	z_as_relative = false
	set_process(true)
	queue_redraw()
