...
const 대상표 := {
	...
	# ★[2026-09-08 STEP 13] STAGE 2 — 지금까지 이 표에 없었다.
	#   (씬에 있던 빛가림 4 개는 `patch_STAGE2_STEP12.gd` 가 씬을 다시 지을 때 같이 만든
	#    것이라, 이 도구로는 관리가 안 되고 있었다. 여기로 옮겨서 **한 곳에서** 관리한다.)
	#
	#   고른 기준 = "이 스테이지에서 그림자를 드리우는 유일한 광원(`복도_창문_주광`)이
	#   실제로 막혀야 하는 건축물". 바깥 껍데기 벽 4 장 + 천장 슬래브 2 장이다.
	#     · 천장_2층복도(x −1700~1450) 가 있어야 그 **오른쪽 끝(1450)** 이 지붕 구멍으로 읽힌다.
	#     · 천장_1층(x 2400~4600) 이 있어야 그 **오른쪽(4600~6000)** 이 1층으로 뚫린 구멍이 된다.
	#   ⚠ 계단참(LANDING_*)·발판·바닥은 **일부러 뺐다.** 계단 C 처럼 가장 어두운 구간의
	#     tread 가 위 계단참 그림자에 통째로 잠기면 낙사 경계가 사라진다(지시서 §16 1순위).
	"res://scenes/집/스테이지_2_복도계단.tscn": [
		# # STEP15: STAGE 2 를 전면 재설계하면서 껍데기 노드 이름이 바뀌었다
	#   (벽_왼위/왼아래 → 벽_왼 하나 · 천장_2층복도 → 천장_2층).
	#   옛 이름을 그대로 두면 "⚠ 없음" 만 찍히고 그림자가 하나도 안 생긴다.
		"지형/벽_왼_SS_BRICK",
		"지형/벽_오른_SS_BRICK",
		"지형/천장_2층_SS_BRICK",
		"지형/천장_1층_SS_BRICK",
