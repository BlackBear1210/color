...
func _오브젝트_이동(루트: Node2D) -> void:
	...
	# 1층 반딧불 무리도 같이.
	# ★[2026-09-08 STEP 13 수정] 예전에는 `홀.position += Vector2(2600, 2230)` 이었다.
	#   두 번 돌리면 그만큼 **또** 밀려나는 누적 코드다(CLAUDE.md §5 위반). 실제로 그래서
	#   자식 구슬들이 월드 (7795, 4890) — 맵(x ≤ 6200 · y ≤ 3400) **밖**으로 나가 있었다.
	#   → 절대 좌표로 못 박는다. 값은 `tools/조명_배치.gd` 의 배치표와 같아야 한다
	#     (그쪽이 조명의 유일한 출처다).
	var 홀 := 오.get_node_or_null("반딧불_홀") as Node2D
	if 홀 != null:
		홀.position = Vector2(1520.0, 2020.0)
	# 1층 조명 3종을 새 1층 위치로.
