...
##   D. 창문커튼 페인트    — 같은 색 낭비 / 반대색 덮어쓰기 / 되돌리기
##   E. 구형 색섞임 옵션과 무관하게 플레이어 페인트는 회색을 만들지 않는다
...
func _창문_페인트() -> void:
	...
	확인("★다른 색을 덮으면 흰색으로 다시 칠해진다", 창.명중(ColorDefs.WHITE, Vector2.ZERO) == "painted")
	확인("덮어쓴 흰색이 남는다", 창.현재색() == ColorDefs.WHITE)
	확인("검정도 다시 덮어쓸 수 있다", 창.명중(ColorDefs.BLACK, Vector2.ZERO) == "painted")
	확인("회수하면 창이 열리고 무색으로 돌아온다", 창.되돌리기())
...
func _창문_빛연동() -> void:
	...
	for i in 6:
		await process_frame
	# 좌·우 커튼 분리 뒤 `_닫힘`은 변수가 아니라 평균을 돌려주는 메서드가 됐다.
	var 닫힘: float = 창.call("_닫힘")
	# 한 발은 좌·우 중 한쪽만 닫으므로 전체 평균 닫힘은 0.5가 정상이다.
	확인("맞은 쪽 커튼 한 장이 닫힌다", 닫힘 > 0.49, "평균 닫힘 %.2f" % 닫힘)
