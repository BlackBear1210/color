...
func _tick() -> void:
	...
	match n:
		...
		10:
			...
			# ★[2026-08-07] 진입구 4 개가 다 있는지 — 버튼이 하나라도 빠지면
			#   그 라인은 게임에서 아예 갈 수 없게 된다(가장 조용히 망가지는 종류).
			var 메뉴 := current_scene.get_node("UILayer/Menu")
			for 버튼이름 in ["StartButton", "SmartWorldButton",
		...
		110:
			...
			var 기대경로: String = (load("res://scenes/lobby/lobby.gd") as GDScript).get("MAIN_SCENE")
			check("3) 시작 목적지 씬으로 바뀌었다 (%s)" % 기대경로.get_file(),
				current_scene != null and current_scene.scene_file_path == 기대경로)
			check("3-2) Player·ProtoCamera 조립 완료",
