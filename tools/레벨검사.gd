...
## x 를 이 간격으로 훑는다. 작을수록 정확하고 느리다.
## ★[2026-09-06] 상수에서 변수로 바꿨다 — `--간격=8` 로 줄일 수 있다.
##   32px 격자는 **틈을 최대 31px 넓게 읽는다**(양 끝이 표본 사이에 걸리므로).
##   실제 120px 짜리 틈이 160px 으로 보고돼 "건널 수 없다" 가 되는 일이 실제로 있었다
##   (하수도 2-3 배수구·관문 4 호퍼 다리). 기본값은 그대로 32 라 기존 검사 결과는 안 바뀐다.
var 표본간격 := 32.0
...
func _init() -> void:
	...
	for a in args:
		if a == "--지도":
			...
			_상세출력 = true
		elif a.begins_with("--간격="):
			표본간격 = maxf(a.substr(5).to_float(), 2.0)
		elif not a.begins_with("--"):
			_씬경로 = a
	if _씬경로.is_empty():
		print("사용법: Godot --headless --path . -s res://tools/레벨검사.gd -- <씬경로> [--지도] [--상세] [--간격=8]")
		quit(2)
		return
...
func _선반_묶기(표면: Array) -> void:
	...
	for seg in _선반:
		# ★간격을 8 로 줄여도 "밟을 수 없는 티끌" 기준은 16px 로 고정한다.
		#   표본간격에 비례시키면 간격 8 에서 4px 짜리 잡음까지 선반으로 올라온다.
		if seg["x1"] - seg["x0"] >= maxf(표본간격 * 0.5, 16.0):
			걸러진.append(seg)
	_선반 = 걸러진
...
func _플레이어_크기_재기() -> void:
	...
	#   못 재면 기본값(97×44)으로 계속 도는데, 그러면 게이트 결과가 거짓이 되므로 알린다.
	var 잰것 := 플레이어몸.재기(p)
	if not 잰것["찾음"]:
		print("  ⚠ 플레이어 콜리전을 못 찾음 → 기본값(키 97 · 폭 44)으로 검사한다")
		return
	_플레이어_폭 = (잰것["크기"] as Vector2).x
	_플레이어_높이 = (잰것["크기"] as Vector2).y
