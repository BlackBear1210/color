...
func _시험_구간별_점프() -> void:
	...
	for c in 구간:
		...
		#
		# ★★[2026-09-06 STEP 8] **기다리기를 `_놓기` 보다 먼저** 옮겼다.
		#   예전 순서(놓기 → 착지 → 기다리기)에는 순서 버그가 있었다:
		#     ① 출발 자리 (2320, −1562) 는 **서랍이 열려 있을 때만** 발판이 있는 자리다
		#        (열림 2297…2597 · 닫힘 1997…2297 · 옷장은 1566…2297 까지).
		#     ② 그래서 놓는 순간 서랍이 닫혀 있으면 플레이어가 1522 아래 **바닥까지
		#        떨어진 채로** 기다리기에 들어가고, 열리고 나서 바닥에서 달려 나간다.
		#        → 결과가 (2489, 0) 로 찍혀 FAIL. 지형 문제가 아니라 시험 순서 문제다.
		#     ③ 서랍 주기는 3 초인데 이 시험 앞의 검사들이 몇 프레임을 쓰느냐에 따라
		#        위상이 달라진다. 즉 **씬에 노드를 더하기만 해도 결과가 뒤집힌다**
		#        (STEP 8 에서 구역 C/D 를 심자 실제로 뒤집혔다).
		#   → 열린 것을 확인한 **뒤에** 놓으면 위상과 무관하게 항상 같은 상태에서 출발한다.
