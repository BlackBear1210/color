...
const 발판들 := [
	["SS_BRICK_LEDGE_A_01", 160.0, -2050.0],
	["SS_BRICK_LEDGE_B_01", 620.0, -1940.0],
	["SS_BRICK_LEDGE_C_01", 1070.0, -1830.0],
	["SS_BRICK_LEDGE_D_01", 1405.0, -1720.0],
	["SS_WOOD_WARDROBE_01", 1930.0, -1522.0],
	["SS_BRICK_SPIKE_BASE_01", 2597.0, -1632.0],
	["SS_BRICK_LEDGE_EXIT_01", 3047.0, -1742.0],
]
...
func _시험_벽충돌() -> void:
	await _놓기(Vector2(-300.0, -2100.0))
	await _착지_대기()
	Input.action_press("move_left")
...
func _시험_천장충돌() -> void:
	await _놓기(Vector2(3047.0, -1790.0))
	await _착지_대기()
	var y0 := _p.global_position.y
	Input.action_press("jump")
	var 최고 := y0
...
func _시험_구간별_점프() -> void:
	...
	var 구간 := [
		["선반A→선반B", 60.0, -2050.0, 320.0, -1940.0, 470.0],
		["선반B→선반C", 500.0, -1940.0, 770.0, -1830.0, 920.0],
		["선반C→선반D", 950.0, -1830.0, 1220.0, -1720.0, 1250.0],
		["선반D→옷장", 1280.0, -1720.0, 1560.0, -1522.0, 1566.0],
		["옷장→가시받침", 1700.0, -1522.0, 2297.0, -1632.0, 2447.0],
		["가시받침→출구", 2477.0, -1632.0, 2747.0, -1742.0, 2897.0],
	]
	var 전부 := true
	var 상세: Array = []
	for c in 구간:
...
func _시험_주파() -> void:
	...
	_기록("1-2 Direction", 최대x > 3150.0,
		"봇이 도달한 최대 x = %.0f (출구선반 2897…3197) · 리스폰 %d 회" % [최대x, 죽음])
...
func _시험_색칠() -> void:
	...
	var 전2: int = 코어.남은_탄약
	코어.발사_소모()
	var 결과2: String = 코어.명중_처리(구조물, ColorDefs.BLACK, Vector2(400.0, 5.0))
	var 후2: int = 코어.남은_탄약
