...
# ------------------------------------------------------------------ 8) 흰색 = 검정 반전
## ⚠★[2026-08-29] 반전은 **밝은 디테일**을 가진 재질(잔디·나무)에만 맞다.
##   벽돌처럼 **줄눈이 바탕보다 어두운** 그림은 반전하면 줄눈이 1.0 이 되어 빛나고,
##   흰 판이 "흰 물감을 부은 것"처럼 읽힌다(성진님 제보로 확인).
##   → 벽돌 계열을 여기서 다시 구웠다면 반드시 이어서 돌릴 것:
##      godot --headless --path . -s res://tools/생성_흰색짝.gd -- ##          --방식=파임유지 --대비=1.6 --덮어쓰기 --폴더=<그 재질 폴더>
##   어느 쪽인지 모르면 `--진단` 으로 먼저 재 본다.
