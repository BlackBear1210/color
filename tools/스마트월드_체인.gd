...
func _실행() -> void:
	...
	# ★[2026-08-18 추가] `-- --만 5,6,7` 로 **일부 스테이지만** 굽는다.
	#   왜 필요한가: 이 도구는 씬을 통째로 다시 만든다. 스테이지 하나를 새로 넣을 때마다
	#   1~4 까지 같이 다시 구워지면 **에디터에서 손본 값이 전부 날아간다**
	#   (CLAUDE.md 규칙 4). 그래서 새 스테이지를 넣을 때는 그것만 굽는다.
	#   인자를 안 주면 예전과 똑같이 전부 굽는다 — 기존 사용법에 회귀가 없다.
	var 대상 := _대상_번호들()
	for s in 챕터.스테이지표:
		if not 대상.is_empty() and not 대상.has(int(s["번호"])):
			continue
		_시도 += 1
		await _스테이지_굽기(s)
...
## `--만 5,6,7` 을 파싱한다. 비어 있으면 "전부" 라는 뜻.
## ⚠ 통로 목적지는 **표 전체**를 보고 계산하므로(`_통로_잇기`), 일부만 구워도
##   앞뒤 연결은 정확하다. 다만 **이웃 스테이지의 입구 통로 위치**는 그 씬을 다시
##   구워야 갱신되므로, 표에서 순서를 바꿨다면 이웃도 함께 지정할 것.
func _대상_번호들() -> Array[int]:
	var 결과: Array[int] = []
	var 인자 := OS.get_cmdline_user_args()
	for i in 인자.size():
		if 인자[i] != "--만" or i + 1 >= 인자.size():
			continue
		for 조각 in String(인자[i + 1]).split(",", false):
			var t := String(조각).strip_edges()
			if t.is_valid_int():
				결과.append(t.to_int())
	if not 결과.is_empty():
		print("[체인] --만 지정: %s (나머지는 건드리지 않는다)" % str(결과))
	return 결과
...
func _장식_깔기(루트: Node2D, 번호: int, 챕터번호: int) -> void:
	...
	# ★[2026-08-19] 집 3장(5 거실·6 굴뚝·7 지붕)은 방별 장식으로 라우팅한다.
	#   숲(그 외)에 집 가구가 새지 않게 스테이지 번호로 방을 정한다.
	#   ⚠ 도형님 요청 "무더기로 뭉치지 말 것" → 집은 밀도를 0.55 → 0.36 으로 낮춰 흩뿌린다.
	var 방 := -1
	var 밀도 := 0.55
	match 번호:
		5: 방 = 배치_S.방_.거실; 밀도 = 0.38
		6: 방 = 배치_S.방_.굴뚝; 밀도 = 0.34
		7: 방 = 배치_S.방_.지붕; 밀도 = 0.38
	var 셈 := 배치_S.깔기(루트, 층, 리밋, 챕터번호, 20260808 + 번호 * 17, 밀도, 방)
	print("   장식: 지면 %d · 늘어짐 %d · 전경 %d (방 %d)"
		% [셈["지면"], 셈["천장"], 셈["전경"], 방])
