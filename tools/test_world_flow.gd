...
func _init() -> void:
	if not _대상_있나(["res://scenes/world_1/world_1.tscn", "res://scenes/world_1/zone_01/zone_01.tscn"]):
...
## ============================================================================
## ⚠[2026-08-20] 이 검사는 **폐기된 라인**을 본다 — 스스로 건너뛴다.
## ----------------------------------------------------------------------------
##   커밋 `df03f3d 폐기 라인 정리` 에서 이 검사가 여는 씬들이 전부 삭제됐다
##   (zone_* · 스테이지_* · 테스트월드제작 — 씬 27개 + 스크립트 7개).
##   씬이 없으니 검사는 **영원히 빨갛다.** 그런데 빨간 검사가 하나라도 상주하면
##   팀이 "원래 하나는 실패해" 라고 넘기기 시작하고, 그때부터 **진짜 실패도 안 보인다.**
##
##   ▣ 왜 파일을 지우지 않았나
##     이 검사에는 도달 가능성 BFS·점프 한계 판정 같은 **쓸 만한 코드**가 들어 있다.
##     스마트월드용으로 옮길 때 참고할 원본이라 남긴다. 대신 대상이 없으면
##     "건너뜀" 을 찍고 **정상 종료(0)** 한다 — 검사판이 초록으로 유지된다.
##   ▣ 되살리려면
##     대상 씬을 복구한 뒤 이 블록만 지우면 예전 그대로 돈다.
## ============================================================================
